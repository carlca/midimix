...
object MidiMixLights:
 ...
 /** Flushes all Solo button lights */    
  ...
  private def flushMuteLight(t: Int): Unit = 
    mHost.getMidiOutPort(0).sendMidi(0x90, 
      Maps.getMuteMidi(t).getOrElse(0x00), 
      if Tracks.getIsMuted(t) then 
        0x00 else 0x7F)

 /** Flushes Solo button light for one track */
  private def flushSoloLight(t: Int): Unit = 
    mHost.getMidiOutPort(0).sendMidi(0x90, Maps.getSoloMidi(t).get, if Tracks.getIsSolo(t) then 0x7F else 0x00);

 /** Flushes Arm button light for one track */    
  private def flushArmLight(t: Int): Unit  = 
    mHost.getMidiOutPort(0).sendMidi(0x90, Maps.getArmMidi(t).get, if Tracks.getIsArmed(t) then 0x7F else 0x00);
